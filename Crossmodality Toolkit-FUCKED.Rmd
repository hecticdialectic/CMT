...
holder1 <- cbind(zvals, pvals, statvals)
EffSizes.Full <- as.data.frame(cbind(comparisons,  holder1 ))
...
## Appendix 1 - Sample Size calculation and Finals Ns


##### This is a Page Break

## Appendix 2- Collapsing Sub Experiments
...
## Appendix 3- Directionality and Symmetricality of Crossmodal Associations
...
for(comparison in comparisons){
  ...
  tval1 <- mean(testdata1$Response)
  tvals1 <- c(tvals1, tval1)
  
  tval2 <- mean(testdata2$Response)
  tvals2 <- c(tvals2, tval2)
  
}

tvalsdiff <- abs(as.numeric(tvals1) - as.numeric(tvals2))

holder1 <- cbind(tvals1, tvals2, tvalsdiff, zvals, pvals, statvals)
EffSizes.Full.Symmetrical <- data.frame(cbind(comparisons,  holder1 ))

```
 One persistent question in the research literature is whether crossmodal associations operate at equal strengths in both directions. For example do people associate bright images with loud sounds to a greater or lesser extent than they associate loud sounds with bright images. Besides the theoretical prudence of considering the importance of the direction of associations, it was important for the purposes of this study that we explore the symmetricality of our association matrix for the outcomes of various statistical tests.
